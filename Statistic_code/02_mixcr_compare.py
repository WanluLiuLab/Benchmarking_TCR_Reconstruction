...
for s,c in [(x,y) for x in samples for y in counts]:
    ...
    # get result
    result_A = pd.read_csv('./data/02_mixcr/01_PE_output/{}/{}_merge{}.clonotypes.TRA.txt'.format(s,s,c),
                            sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[5,7,23])
    result_B = pd.read_csv('./data/02_mixcr/01_PE_output/{}/{}_merge{}.clonotypes.TRB.txt'.format(s,s,c),
                            sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[5,7,23])
...
for s,c in [(x,y) for x in samples for y in counts]:
    print('comparing sample: '+s+' count: '+str(c)+' input type: '+data_type+'...')
    # get result
    result_A = pd.read_csv('./data/02_mixcr/01_SE_output/{}/{}_merge{}.clonotypes.TRA.txt'.format(s,s,c),
                            sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[5,7,23])
    result_B = pd.read_csv('./data/02_mixcr/01_SE_output/{}/{}_merge{}.clonotypes.TRB.txt'.format(s,s,c),
                            sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[5,7,23])
    result = pd.concat([result_A,result_B])
    for i in ['V', 'J']:
        result[i] = result[i].str.replace('\*.*', '', regex = True)   
