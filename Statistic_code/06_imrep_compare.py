...
for s,c in [(x,y) for x in samples for y in counts]:
    ...
    # get result
    result = pd.read_csv('./data/06_imrep/01_bam_output/{}/{}_merge{}.cdr3'.format(s,s,c),
                            sep = ',',header=0,names=['CDR3aa','V','J'],usecols=[0,3,5])
...
for s,c in [(x,y) for x in samples for y in counts]:
    print('comparing sample: '+s+' count: '+str(c)+' input type: '+data_type+'...')
    # get result
    result = pd.read_csv('./data/06_imrep/01_SE_output/{}/{}_merge{}.cdr3'.format(s,s,c),
                        sep = ',',header=0,names=['CDR3aa','V','J'],usecols=[0,3,5])
