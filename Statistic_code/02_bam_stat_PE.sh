...
filelist=$(cat file_head_PE.txt)
for filehead in $filelist
do
	###01 candidate reads
	rm ./02_bam_statistic/${filehead}/01_totalreads_count.txt
	mkdir ./02_bam_statistic/${filehead}
	wc -l 01_bam_output/${filehead}/*toassemble_1.fq | awk -F " " '{print $1}' > ./02_bam_statistic/${filehead}/01_candidatereads_count.txt
	sed -i '$d' ./02_bam_statistic/${filehead}/01_candidatereads_count.txt
	wc -l 01_bam_output/${filehead}/*toassemble_1.fq | awk -F "." '{print $2}' > ./02_bam_statistic/${filehead}/01_cellbarcode.txt
