...
for s,c in [(x,y) for x in samples for y in counts]:
    print('comparing sample: '+s+' count: '+str(c)+' input_type: '+data_type+'...')
    # get result
    result = pd.read_csv('./data/08_derr/01_PE_output/{}/{}_merge{}_results.tsv'.format(s,s,c),sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[0,1,3])
    for i in ['V', 'J']:
...
for s,c in [(x,y) for x in samples for y in counts]:
    print('comparing sample: '+s+' count: '+str(c)+' input_type: '+data_type+'...')
    # get result
    result = pd.read_csv('./data/08_derr/01_SE_output/{}/{}_merge{}_results.tsv'.format(s,s,c),sep = '\t',header=0,names=['V','J','CDR3nt'],usecols=[0,1,3])
    for i in ['V', 'J']:
        result[i] = result[i].str.replace('\*.*', '', regex = True)    
    # get ground truth
    ground_truth_df_VJ,ground_truth_df_VDJ = query_ground_truth(s, c)
